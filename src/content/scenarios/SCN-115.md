---
id: SCN-115
title: An NGO distributes an offline field library
...
preview_summary: An NGO distributes an offline field library. Knowledge needs to reach people beyond the original working group. Koali keeps the public explanation, learning resource or media object connected to its source knowledge, provenance and revisions while it is adapted for new audiences.
preview_image: /scenarios/images/scenario_115.png
preview_image_alt: 'Abstract illustration for SCN-115: An NGO distributes an offline field library'
badges:
- FIELD
- OFFLINE
related:
...
- SCN-100
---
# SCN-115 — An NGO distributes an offline field library
...
An NGO distributes an offline field library. Knowledge needs to become a usable public or learning format—course, guide, video, audio, library, or channel—without becoming disconnected from its source.
...
`corpus → pack → transport → local use → new evidence → sync`
...
The value is not any one isolated feature. It is preserving the same context across **corpus → … → sync** so knowledge, people, choices, work, and memory remain connected.
